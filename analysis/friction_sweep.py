...
MUS=[1.0489,0.9,0.8,0.7,0.6,0.5,0.4]
VG=0.7
print("Friction sweep at vgain=%.1f  (dry mu=1.05)\n"%VG)
print(f"{'mu':>6} | {'pure pursuit':>22} | {'stanley':>22} | {'mpc (assumes dry)':>22} | {'mpc (knows mu)':>22}")
print(f"{'':>6} | {'res  mean   max':>22} | {'res  mean   max':>22} | {'res  mean   max':>22} | {'res  mean   max':>22}")
for mu in MUS:
    P=dict(BASE); P["mu"]=mu
    ...
    for c in (PurePursuit(vgain=VG), Stanley(vgain=VG),
              MPCDyn(vgain=VG,mu_model=1.0489), MPCDyn(vgain=VG,mu_model=mu)):
